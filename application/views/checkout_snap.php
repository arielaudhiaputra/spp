          ...
          <div class="card">
            <div class="card-header d-block">
              ...
              <div class="d-inline ml-auto float-right" style="margin: 10px;">
                <h5> Belum dibayar : Rp. <?= number_format($nominal) ?></h5>
              </div>
            </div>
...
<input type="hidden" name="result_data" id="result-data" value=""></div>
    ...
    const nominal =  '<?= $nominal ?>';
      ...
      if ($('#bulan_bayar').val() != '' && $('#jumlah_bayar').val() != '') {

        if (nominal == 0) {
          alert('Sudah Lunas')
        } else if ($('#jumlah_bayar').val() <= nominal) {
        ...
        alert('Jumlah Pembayaran anda terlalu besar');
        }
